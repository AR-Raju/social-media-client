...
import { Home, Users, MessageCircle, Users2, Bookmark, Calendar, Settings, TrendingUp } from "lucide-react"
  ...
  return (
    <aside className="fixed left-0 top-16 h-[calc(100vh-4rem)] w-64 border-r bg-background overflow-y-auto custom-scrollbar hidden lg:block">
      <div className="p-4 space-y-6">
        ...
        {/* Friend Suggestions */}
        {friendSuggestions?.data?.data && friendSuggestions.data.data.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-muted-foreground">People you may know</h3>
            <div className="space-y-2">
              {friendSuggestions.data.data.slice(0, 3).map((friend: any) => (
                <div key={friend._id} className="flex items-center space-x-2 p-2 rounded-lg hover:bg-muted/50">
        ...
        {/* Group Suggestions */}
        {groupSuggestions?.data?.data && groupSuggestions.data.data.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-muted-foreground">Suggested Groups</h3>
            <div className="space-y-2">
              {groupSuggestions.data.data.slice(0, 3).map((group: any) => (
                <div key={group._id} className="flex items-center space-x-2 p-2 rounded-lg hover:bg-muted/50">
                  <Avatar className="h-8 w-8">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{group.membersCount} members</p>
                  </div>
                </div>
              ))}
            </div>
