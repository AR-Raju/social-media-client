...
import { notificationsApi, groupsApi } from "@/services/api"
          ...
          <CardContent className="space-y-3">
            {recentNotifications?.data?.data?.length > 0 ? (
              recentNotifications.data.data.map((notification: any) => (
                <div key={notification._id} className="flex items-start space-x-2 p-2 rounded-lg hover:bg-muted/50">
                  <Avatar className="h-8 w-8">
                    <AvatarImage
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {trendingGroups?.data?.data?.length > 0 ? (
                ...
                <div key={group._id} className="flex items-center space-x-2 p-2 rounded-lg hover:bg-muted/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs text-muted-foreground">{group.membersCount} members</span>
                    </div>
                  </div>
              ...
              <p className="text-sm text-muted-foreground">No trending groups</p>
            )}
          </CardContent>
        </Card>
