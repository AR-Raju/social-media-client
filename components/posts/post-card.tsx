...
export function PostCard({ post }: PostCardProps) {
  ...
  const reactMutation = useMutation({
    mutationFn: ({ postId, type }: { postId: string; type: string }) => postsApi.reactToPost(postId, type),
    onSuccess: () => {
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2 mb-2">
                <Avatar className="h-6 w-6">
                  ...
                  />
                  <AvatarFallback>{post.sharedPost.author.name.charAt(0)}</AvatarFallback>
                </Avatar>
                <span className="text-sm font-medium">{post.sharedPost.author.name}</span>
              </div>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center space-x-1">
            {Object.entries(reactionIcons).map(([type, Icon]) => (
              ...
              >
                {typeof Icon === "string" ? <span className="mr-1">{Icon}</span> : <Icon className="mr-1 h-4 w-4" />}
